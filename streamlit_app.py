...
SIGMA_UNIVERSAL_GRAINS_8_10 = [8.0, 9.0, 10.0]
...
def fit_sigma_universal_grain_size_model(
    ...
    if variant == "median_constants":
        allowed_grains = SIGMA_UNIVERSAL_GRAINS
    elif variant == "linear_weighted_constants_8_10":
        allowed_grains = SIGMA_UNIVERSAL_GRAINS_8_10
    else:
        raise ValueError(f"Неизвестный вариант универсальной sigma-модели: {variant}")

    for grain, result in cleaned_results.items():
        if float(grain) not in allowed_grains:
            continue
        grain_size = GRAIN_SIZE_MM.get(float(grain))
        if grain_size is None:
            continue
    ...
    ln_g = coeff_df["ln_grain_size"].to_numpy(dtype=float)
    weights = np.clip(coeff_df["R²"].to_numpy(dtype=float), 0.0, None)
    if not np.any(weights > 0):
        weights = np.ones(len(coeff_df), dtype=float)
    ...
    if variant == "median_constants":
        ...
        p_rule_text = f"p={p_const:.10f} (медиана по локальным моделям)"
        m_rule_text = f"m={m_const:.10f} (медиана по локальным моделям)"
    else:
        X = np.column_stack([np.ones(len(coeff_df)), ln_g])
        model_log_a = sm.WLS(coeff_df["log_a"], X, weights=weights).fit()
        params_a_linear = np.asarray(model_log_a.params, dtype=float)
        params_a = np.array([params_a_linear[0], params_a_linear[1], 0.0], dtype=float)
        p_const = float(np.average(coeff_df["p_exp"].to_numpy(dtype=float), weights=weights))
        m_const = float(np.average(coeff_df["m_exp"].to_numpy(dtype=float), weights=weights))
        variant_label = "зерна 8–10: log(A)(dg) = u0 + u1·ln(dg); p и m = weighted mean"
        variant_title = "Зерна 8–10: log(A) линейно, p/m взвешенно"
        p_rule_text = f"p={p_const:.10f} (взвешенное среднее по R² локальных моделей)"
        m_rule_text = f"m={m_const:.10f} (взвешенное среднее по R² локальных моделей)"
    ...
    params = {
        ...
        "allowed_grains": [float(g) for g in allowed_grains],
    }
    included_grains = ", ".join(str(int(g)) if float(g).is_integer() else str(g) for g in coeff_df["G"].tolist())
    if variant == "median_constants":
        log_a_text = f"log(A)(dg)=alpha0+alpha1·ln(dg)+alpha2·[ln(dg)]², R²={model_log_a.rsquared:.4f}"
    else:
        log_a_text = f"log(A)(dg)=alpha0+alpha1·ln(dg), R²={model_log_a.rsquared:.4f}"
    summary_text = (
...
def evaluate_sigma_universal_model(params: dict[str, float], cleaned_results: dict[float, FitResult]) -> dict[str, float]:
    ...
    total_points = 0
    total_models = 0
    allowed_grains = {float(g) for g in params.get("allowed_grains", SIGMA_UNIVERSAL_GRAINS)}
    for grain, result in cleaned_results.items():
        if float(grain) not in allowed_grains:
            continue
        grain_size = GRAIN_SIZE_MM.get(float(grain))
...
def get_final_sigma_params(params: dict[str, float]) -> dict[str, float]:
    rounded = rounded_copy(params, 2, ["alpha0", "alpha1", "alpha2", "p_const", "m_const"])
    if "allowed_grains" in params:
        rounded["allowed_grains"] = list(params["allowed_grains"])
    return rounded


def sigma_universal_variant_options() -> list[tuple[str, str]]:
    return [
        ("median_constants", "Все зерна 3, 5, 8, 9, 10: log(A) квадратично, p/m = медианы"),
        ("linear_weighted_constants_8_10", "Только зерна 8, 9, 10: log(A) линейно, p/m = взвешенные"),
    ]


def build_sigma_universal_payload(cleaned_sigma_results: dict[float, FitResult], variant: str) -> dict[str, object]:
    params_item, coeff_df_item, summary_item = fit_sigma_universal_grain_size_model(cleaned_sigma_results, variant=variant)
    ...
    return {
        "key": variant,
        "title": str(params_item["variant_title"]),
...
def render_universal_models_tab(prepared_df: pd.DataFrame, valid_grains: list[float]) -> None:
    ...
    try:
        ...
        sigma_variant_choice = st.selectbox(
            "Вариант универсальной sigma-модели",
            options=[key for key, _ in sigma_universal_variant_options()],
            format_func=lambda key: dict(sigma_universal_variant_options())[key],
            key="sigma_universal_variant_compare",
        )
        selected_sigma_variant = build_sigma_universal_payload(cleaned_sigma_results, sigma_variant_choice)
    except Exception as exc:
    ...
    with col_mid:
        ...
        if selected_sigma_variant is None:
            ...
            if selected_sigma_variant["key"] == "linear_weighted_constants_8_10":
                log_a_formula = f"log(A)(dg) = {fmt_trimmed(p['alpha0'], 2)} + ({fmt_trimmed(p['alpha1'], 2)}) · ln(dg)"
            else:
                log_a_formula = f"log(A)(dg) = {fmt_trimmed(p['alpha0'], 2)} + ({fmt_trimmed(p['alpha1'], 2)}) · ln(dg) + ({fmt_trimmed(p['alpha2'], 2)}) · [ln(dg)]²"
            st.code(
                "\n".join(
                    [
                        "cσ = A(dg) · τ^p · ((T - 550) / 350)^m",
    ...
    if not valid_grains:
        ...
        with universal_tab:
            ...
            st.write(
                ...
                "для каждого номера зерна, затем для log(A) берется фиксированная форма по выбранному сценарию, "
                "а p и m заменяются на константы. Здесь можно сравнить классический вариант по всем зернам "
                "и отдельный вариант только для зерен 8, 9 и 10."
            )
            try:
                cleaned_sigma_results = build_cleaned_sigma_grain_results(prepared_df, valid_grains)
                ...
                sigma_variant_choice = st.selectbox(
                    "Сценарий универсальной sigma-модели",
                    options=[key for key, _ in sigma_universal_variant_options()],
                    format_func=lambda key: dict(sigma_universal_variant_options())[key],
                    key="sigma_universal_variant_sigma_tab",
                )
                sigma_payload = build_sigma_universal_payload(cleaned_sigma_results, sigma_variant_choice)
                selected_params = sigma_payload["params"]
                sigma_coeff_df = sigma_payload["coeff_df"]
                sigma_summary = sigma_payload["summary"]
                sigma_eval = sigma_payload["eval"]
                ...
                if sigma_variant_choice == "linear_weighted_constants_8_10":
                    st.info(
                        f"Используются только зерна 8, 9 и 10. Для log(A) берется линейная зависимость от ln(dg), "
                        f"а p и m считаются как взвешенные средние по R² очищенных локальных моделей. "
                        f"Для выбранного варианта: RMSE={sigma_eval['RMSE по T, °C']:.4f} °C, R²={sigma_eval['R² по T']:.4f}."
                    )
                    log_a_formula = f"log(A)(dg) = {fmt_trimmed(selected_params['alpha0'], 2)} + ({fmt_trimmed(selected_params['alpha1'], 2)}) · ln(dg)"
                else:
                    st.info(
                    ...
                    log_a_formula = f"log(A)(dg) = {fmt_trimmed(selected_params['alpha0'], 2)} + ({fmt_trimmed(selected_params['alpha1'], 2)}) · ln(dg) + ({fmt_trimmed(selected_params['alpha2'], 2)}) · [ln(dg)]²"
                ...
                with st.form(key="sigma_universal_form"):
                    c1, c2, c3 = st.columns(3)
                    allowed_sigma_grains = [g for g in sorted(GRAIN_SIZE_MM.keys()) if float(g) in set(selected_params.get("allowed_grains", GRAIN_SIZE_MM.keys()))]
                    with c1:
