...
def sigma_universal_temperature_report_plot(df: pd.DataFrame, title: str) -> None:
    plot_df = df.dropna(subset=["T", "T_pred_universal", "c_sigma"]).copy()
    if plot_df.empty:
        st.info("Нет данных для графической интерпретации универсальной sigma-модели.")
        return

    fig, axes = plt.subplots(1, 2, figsize=(13, 5), constrained_layout=True)

    low = float(min(plot_df["T"].min(), plot_df["T_pred_universal"].min()))
    high = float(max(plot_df["T"].max(), plot_df["T_pred_universal"].max()))
    scatter = axes[0].scatter(
        plot_df["T"],
        plot_df["T_pred_universal"],
        c=plot_df["c_sigma"],
        cmap="plasma",
        s=80,
        alpha=0.85,
    ...
    axes[0].plot([low, high], [low, high], linestyle="--", color="gray", linewidth=1.5, label="Идеальное совпадение")
    axes[0].set_xlabel("Экспериментальная температура, °C")
    axes[0].set_ylabel("Расчетная температура, °C")
    axes[0].set_title("Температура: факт vs универсальная модель")
    axes[0].legend(loc="best")
    cbar = fig.colorbar(scatter, ax=axes[0])
    cbar.set_label("Содержание σ-фазы, %")

    axes[1].axhline(0, linestyle="--", color="gray", linewidth=1.2)
    axes[1].scatter(
        plot_df["c_sigma"],
        plot_df["error_celsius_universal"],
        c=plot_df["T"],
        cmap="viridis",
        s=80,
        alpha=0.85,
    ...
    axes[1].set_xlabel("Содержание σ-фазы, %")
    axes[1].set_ylabel("Ошибка (эксперимент - модель), °C")
    axes[1].set_title("Остатки по содержанию σ-фазы")
...
def build_sigma_universal_evaluation_df(params: dict[str, float], cleaned_results: dict[float, FitResult]) -> pd.DataFrame:
    ...
    for grain, result in cleaned_results.items():
        ...
        if grain_size is None:
            continue
        df = result.data.copy()
        df["grain_size_mm"] = float(grain_size)
        ...
        df["error_celsius_universal"] = df["T"] - df["T_pred_universal"]
        df["abs_error_universal"] = df["error_celsius_universal"].abs()
        rows.append(df)
    if not rows:
...
def evaluate_sigma_universal_model(params: dict[str, float], cleaned_results: dict[float, FitResult]) -> dict[str, float]:
    eval_df = build_sigma_universal_evaluation_df(params, cleaned_results)
    errors = eval_df["error_celsius_universal"]
    return {
...
def render_report_data_tab(prepared_df: pd.DataFrame, valid_grains: list[float]) -> None:
    ...
    sigma_eval_df = build_sigma_universal_evaluation_df(sigma_params, cleaned_sigma_results)
    sigma_universal_df = pd.DataFrame(
        [
            {
    ...
    st.markdown("**4а. Графическая интерпретация универсальной sigma-модели**")
    st.caption("Рекомендуемый график для отчета: сравнение экспериментальной и расчетной температуры с контролем остатков по содержанию σ-фазы.")
    sigma_universal_temperature_report_plot(
        sigma_eval_df,
        "Универсальная степенная sigma-модель: температурная интерпретация по проценту σ-фазы",
    )
...
def render_universal_models_tab(prepared_df: pd.DataFrame, valid_grains: list[float]) -> None:
    ...
    try:
        ...
        selected_sigma_variant = {
            ...
            "eval": eval_item,
            "eval_df": build_sigma_universal_evaluation_df(final_sigma_params, cleaned_sigma_results),
        }
    except Exception as exc:
    ...
    if selected_sigma_variant is not None:
        st.subheader("Графическая интерпретация универсальной sigma-модели")
        st.caption(
            "Слева — совпадение экспериментальной и расчетной температур, справа — остатки модели по содержанию σ-фазы. "
            "Такой график обычно хорошо смотрится в отчете, потому что одновременно показывает точность и возможный систематический сдвиг по проценту фазы."
        )
        sigma_universal_temperature_report_plot(
            selected_sigma_variant["eval_df"],
            "Универсальная степенная sigma-модель по проценту фазы",
        )
